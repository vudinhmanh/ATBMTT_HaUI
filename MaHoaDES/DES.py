...
def message_input():
    while True:
        into = str(input("Please, input messages = "))
        if len(into) <= 8:
            return into
        print("Value invalid, please input sequence length <= 8")


def key_input():
    while True:
        into = str(input("Please, input key = "))
        if len(into) <= 8:
            return into
        print("Value invalid, please input sequence length <= 8")
...
def print_key(key):
    for i in range(16):
        print(f"key[{i}]: {"".join([str(w) for w in key[i]])}")
...
def binary_text(x):
    ...
    for i in range(0, n, 8):
        end += chr(binary_number(x[i:i + 8].zfill(8)))
    return end
...
def encryption(message, key):
    ...
    for i in range(16):
        ...
        x = xor(swap(right, E), key[i])
        s = []
        for j in range(0, 48, 6):
            block = "".join(str(i) for i in x[j:j + 6])
            row = binary_number(block[0] + block[-1])
            col = binary_number(block[1:-2])
            si = number_binary(S[int(j / 6)][row][col]).zfill(4)
            s += [int(w) for w in si]
        right = xor(temp, swap(s, P))
    ip_2 = swap(right + left, IP_2)
    end = "".join([str(i) for i in ip_2])
    return binary_text(end)
...
def decryption(message, key):
    ...
    return binary_text(end.lstrip("0"))
...
def menu():
    ...
    key = 0
    while True:
        print("1. Input messages and key\n"
        ...
        match opt:
            case 1:
                ...
                key = create_key(text_binary(key_input()))
                print_key(key)
            case 2:
